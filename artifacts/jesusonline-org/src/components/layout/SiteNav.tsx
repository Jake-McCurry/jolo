    ...
    <>
      ...
      {open && (
        <div
          ...
          style={{ background: "rgba(6,54,144,0.25)" }}
      ...
      >
        <ul className="max-w-5xl mx-auto px-5 py-4 flex flex-col divide-y divide-white/10">
          {NAV_LINKS.map((link) => (
            <li key={link.href}>
              <a
                href={link.href}
                ...
                rel="noopener noreferrer"
                onClick={() => setOpen(false)}
                className="block py-3.5 text-white font-semibold text-base tracking-wide hover:text-[#0095FF] transition-colors focus-visible:outline-2 focus-visible:outline-white"
              >
                {link.label}
              </a>
            </li>
